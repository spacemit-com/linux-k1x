...
struct cmdlist_header {
	...
	u64 reserved0      : 6;
	u64 nod_len        : 16;
	u64 list_tag       : 1;  /* if the cmdlist is last one tag = 1, otherwise tag = 0 */
	u64 reserved1      : 7;
	u64 reserved2      : 64;
};
...
struct cmdlist_row {
	...
	u32 module_cfg_num    : 2;  /* the last row must 1-2, otherwise 1-3. */
	u32 module_cfg_strobe : 12;
	u32 row_eof_tag       : 2;  /* the last row tag = 3, otherwise tag = 0. */
	u32 module_regs[3];
};
